...
import { Gauge, ArrowDown, ArrowUp, Filter, Plus, LogOut, Shield, Trash2, Download, FileText, Printer, X } from "lucide-react";
...
function ReportModal({ cityId, onClose }: { cityId: number; onClose: () => void }) {
    const [report, setReport] = useState<ReportOut | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetchJSON<ReportOut>(`${API_BASE}/cities/${cityId}/report`)
            .then(setReport)
            .catch(console.error)
            ...
            >
                <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center print:hidden">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handlePrint} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg" title="Print">
                            <Printer className="w-5 h-5" />
                        </button>
                        <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg" title="Close">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                </div>
                ...
                <div className="p-8 print:p-0">
                    {loading ? (
                        <div className="text-center py-12 text-gray-500">Генерація звіту...</div>
                    ) : report ? (
                        <div className="space-y-8">
                            <div className="text-center border-b border-gray-100 pb-6">
                                <h1 className="text-2xl font-bold mb-2">{report.city}</h1>
                                <p className="text-gray-500">Звіт сформовано: {new Date().toLocaleDateString()}</p>
                            </div>

                            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-6 border border-gray-100 dark:border-gray-700">
                                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Загальний стан</h3>
                                <div className="flex items-center gap-4">
                                    <div className="w-3 h-12 rounded-full" style={{ backgroundColor: report.status.color }} />
                                    <div>
                                        <div className="text-3xl font-bold" style={{ color: report.status.color }}>{report.status.status}</div>
                                        <div className="text-gray-600 dark:text-gray-300">{report.status.description}</div>
                                    </div>
                                </div>
                            </div>

                            <div>
                                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Детальна статистика (30 днів)</h3>
                                <div className="grid grid-cols-1 gap-4">
                                    {report.stats.map((s) => (
                                        <div key={s.pollutant} className="flex items-center justify-between p-4 bg-white dark:bg-darkBg border border-gray-100 dark:border-gray-700 rounded-xl">
                                            <div className="font-semibold text-lg uppercase">{s.pollutant}</div>
                                            <div className="flex gap-8 text-sm">
                                                ...
                                                <div className="text-center">
                                                    ...
                                                    <div className="font-bold text-emerald-600">{s.avg}</div>
                                                </div>
                                                <div className="text-center">
                            ...
                            <div className="text-xs text-gray-400 text-center pt-8 border-t border-gray-100">
                                Згенеровано автоматично системою MonAI
                            </div>
                        </div>
                    ) : (
                        <div className="text-center text-red-500">Не вдалося завантажити звіт</div>
                    )}
                </div>
            </motion.div>
        </div>
...
export default function Dashboard() {
    ...
    const downloadCSV = () => {
        ...
        // Headers
        const headers = ["Date", "City", "Station", "Pollutant", "Value"];
    ...
    return (
        <div className={`min-h-screen ${darkMode ? "bg-darkBg text-darkText" : "bg-gray-50 text-gray-800"}`}>
                ...
                <Card className="bg-white dark:bg-darkCard border border-gray-100 dark:border-gray-700">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            className="text-sm px-3 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        <button
                            ...
                            disabled={!cityId}
                            className="text-sm px-3 py-2 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded-xl flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Згенерувати звіт"
                        >
                            <FileText className="w-4 h-4" /> Звіт
                        </button>
                    </div>
                </Card>
            ...
            {showReport && cityId && (
                <ReportModal cityId={Number(cityId)} onClose={() => setShowReport(false)} />
            )}
        </div>
    );
}
